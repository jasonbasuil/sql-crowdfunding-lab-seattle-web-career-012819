...
"SELECT projects.title, SUM(pledges.amount) FROM projects JOIN pledges WHERE projects.id = pledges.project_id GROUP BY title"
end

def selects_the_user_name_age_and_pledge_amount_for_all_pledges_alphabetized_by_name
"SELECT users.name, users.age, SUM(pledges.amount) FROM users JOIN pledges WHERE users.id = pledges.user_id GROUP BY name"
end

def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
"SELECT projects.title, SUM(amount) - projects.funding_goal FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING SUM(amount) >= projects.funding_goal ORDER BY amount"
end
